...
function capture(t) {
    let object = {};
    object.visible = visible(t);
    object.children = [];
    let children = get(t).children;
    for (let i = 0; i < children.length; i++) {
        object.children.push(capture(children[i]));
    }
    return object;
}

function decapture(t, s) {
    if (s.visible)
        show(t);
    else
        hide(t);
    for (let i = 0; i < t.children.length; i++) {
        decapture(t.children[i], s.children[i]);
    }
}

window.onpopstate = function (event) {
    if (event.state !== null)
        decapture(document.body, event.state);
};
...
function page(target) {
    history.replaceState(capture(document.body), null);
    ...
    history.pushState(capture(document.body), null);
}
